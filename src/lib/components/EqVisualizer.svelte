<script lang="ts">
	...
	let { audioContext, analyser, barCount = 32, height = 200, barColor = '#3b82f6' }: Props = $props();
	...
	onMount(() => {
		...
		const draw = () => {
			...
			// Extract frequency bin data from analyser
...
<canvas bind:this={canvas} class="w-full bg-white rounded border border-gray-200" style="height: {height}px"></canvas>
